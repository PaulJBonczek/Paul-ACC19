...
\begin{proof}
With the vehicle at the state $\bm{p}(k) \neq \bm{p}_o$, there exists a reference velocity $r_v(k)$ such that the vehicle's next state $\bm{p}(k+1)$ will never overlap into an undesired state $\bm{p}_o$. Using \eqref{eq:N}, the required number of data samples are computed to guarantee the confidence region \eqref{Confidence_region_updated} of the next state $\bm{p}(k+1)$ will never intersect with an undesired state:
\begin{equation}
