...
\begin{lemma} 
	\label{lemma_1}
	Given $\bm{s}'$ from $\bm{y}$, the system is able to maintain tracking before, during, and after a sensor spoof while guaranteeing stability.
...
Due to uncertainty of the measurements, the confidence region can quickly shift as new measurements are sampled. To counteract this shifting, we propose a boundary around the confidence region,
    \begin{equation}
    \label{eq:Delta}
	    \Delta(k) = 2\big(c_p\frac{\sigma_p}{\sqrt{N_e}} + \epsilon_{v,\theta}\big)
	\end{equation}
which is the maximum distance the region can shift at any time.

To update the confidence region, we compute the distance the vehicle can travel to the next time interval $d_x$. At the next predicted state $\bm{x}^*(k+1)$, if an undesired region is within $C_{\bar{\bm{p}}^*|N_e} + \Delta(k)$, the next value of $N_e$ is computed by,
    \begin{equation}
	    N_e = round \left(c_p \frac{ \sigma_p }{ {\min \lVert \bm{x}_o - \bar{\bm{p}}^* \rVert} -\Delta(k) } \right)^2
	\end{equation}
where $\min \lVert \bm{x}_o - \bar{\bm{p}}^* \rVert$ is the minimum distance from an undesired state to $\bar{\bm{p}}$. Due to the fact that $N_e$ represents the number of samples needed to reduce the estimation region to a desired size, we simply round $N_e$ up to the nearest integer to give us $N_e \in \N$.
...
An update of the reference velocity needs to occur when these undesired regions are within the entire confidence estimation region $ C_{\bar{\bm{p}}^*|N_e} +C_{v,\theta_h|N_e}^{*}+\Delta(k)$ when $N_e=1$ to ensure the vehicle slows to capture more previous data samples with less estimation error. This reference velocity $r_v(k)$ update is calculated by,
    \begin{equation}
        r_v(k) = \min\{\Tilde{r},\bar{r}\}
	   % r_v(k)=r_v(k-1) \left[ \frac{ \min \lVert \bm{x}_o - \bar{\bm{p}}^* \rVert - C_1}{C_2 - C_1} \right]
	\end{equation}
where $r_v(k)$ is the minimum value between,
	\begin{equation}
    \begin{split}
	    \Tilde{r} = \frac{d_x}{N_et_s} \\
	    \bar{r} = \frac{\delta_{\Delta}}{t_s}
	\end{split}
	\end{equation}
	\begin{equation}
	    \delta_{\Delta} = \min \lVert \bm{x}_o \rVert - C_{\bar{\bm{p}}^*|N_e}
	\end{equation}
Doing this computes the reference signal to the lowest velocity, ensuring a velocity to safely navigate.
