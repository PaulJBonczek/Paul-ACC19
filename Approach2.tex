...
\begin{proof}
With the vehicle at a state $\bm{p}(k) \neq \bm{p}_o$, there exists a reference velocity $r_v(k)$ such that the vehicle's next state $\bm{p}(k+1)$ will never enter an undesired state $\bm{p}_o$. Using \eqref{eq:N}, the required number of data samples are computed to guarantee the confidence region \eqref{Confidence_region_updated} of the next state $\bm{p}(k+1)$ will never intersect with an undesired state:
\begin{equation}
\label{lemma_2_eq1}
    C_{\bar{\bm{p}}^*(k+1)|N} \cap \bm{p}_o = \emptyset \nonumber
\end{equation}
With the knowledge of the closest distance between an undesired state $p_o$ and confidence region \eqref{Confidence_region_updated} at time interval $k$, the reference velocity \eqref{eq:ref_vel} is computed such that the vehicle will not travel more than a distance $d_p$ to avoid an undesired state,
\begin{equation}
\label{lemma_2_eq2}
