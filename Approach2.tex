...
We update the estimate $\bm{\theta}^T(k)$ of the true parameter vector $\bm{\theta}_0$ using a \textit{Modified Projection Algorithm}:
	...
	\end{equation}
	\begin{equation}
	\bm{e}(k)=E(q^{-1})y_i(k)-\theta^T(k-1)\phi(k-d)
	\end{equation}
	\begin{align*}
...
For detection, a difference matrix $\Psi(k)$ is designed for every time interval $k$ of each controlled state. The structure of this matrix is,
	...
	\bm{\Psi}(k)=\begin{bmatrix} \psi_{1,1}(k) & \psi_{1,2}(k) & \dots & \psi_{1,M}(k) \\ \psi_{2,1}(k) & \psi_{2,2}(k) &  &  \\ \vdots &  & \ddots &  \\ \psi_{M,1}(k) &  &  & \psi_{M,M}(k) \end{bmatrix}
	\end{equation}
where $\bm{\Psi}(k) \in \R^{M\times M}$ with $M$ is the number of measurements from the available measurement set $\mathcal{S} \in \R^M$. Each element within $\bm{\Psi}(k)$ is represented as the absolute difference between two temporary inputs.
    \begin{equation}
...
We take an $l_1$ vector norm of the $i^{th}$ row from the difference matrix \eqref{eq:error_matrix} which gives us the following column vector:
    \begin{equation}
	\bm{\Psi^{'}}(k)=\begin{bmatrix} \lVert{\bm{\Psi}_1(k)}\rVert_1 \\ \lVert{\bm{\Psi}_2(k)}\rVert_1 \\ \vdots \\ \lVert{\bm{\Psi}_M(k)}\rVert_1 \end{bmatrix}, \in \R^M
	\end{equation}
Next, another column vector $\breve{\bm{\Psi}}(k) \in \R^M$ is created with every element equal to $\inf \bm{\Psi}^{'}(k)$. We now define the error vector as:
    \begin{equation}
    \label{eq:Psi2}
	\bm{\Psi^{''}}(k)=\bm{\Psi^{'}}(k)-\breve{\bm{\Psi}}(k)=\begin{bmatrix} \lVert{\bm{\Psi}_1(k)}\rVert_1 - \inf \bm{\Psi}^{'}(k)\\ \lVert{\bm{\Psi}_2(k)}\rVert_1 - \inf \bm{\Psi}^{'}(k) \\ \vdots \\ \lVert{\bm{\Psi}_M(k)}\rVert_1 - \inf \bm{\Psi}^{'}(k) \end{bmatrix}
	\end{equation}
...
Under non-ideal conditions where uncertainties exist, the elements of the error vector will not remain zero while sensors are uncompromised. By setting a threshold value $\delta$, we say all elements of the error vector need to stay below this level. By refining \eqref{eq:Psi2} for non-ideal conditions we get,
	...
	\bm{\Psi^{''}}(k)&=\bm{\Psi^{'}}(k)-\breve{\bm{\Psi}}(k) \\
	& =\begin{bmatrix} \lVert{\bm{\Psi}_1(k)}\rVert_1 - \inf \bm{\Psi}^{'}(k)\\ \lVert{\bm{\Psi}_2(k)}\rVert_1 - \inf \bm{\Psi}^{'}(k) \\ \vdots \\ \lVert{\bm{\Psi}_M(k)}\rVert_1 - \inf \bm{\Psi}^{'}(k) \end{bmatrix} \leq \bm{\delta}
	\end{split}
	\end{align}
with the vector $\bm{\delta} \in \R^M$ has elements all equal to $\delta$. When an $i^{th}$ element of $\bm{\Psi^{''}}(k)$ breaks the $\delta$ threshold, the corresponding $i^{th}$ measurement is placed into the set $\mathcal{S}_a$ and no longer used by the system. The updated measurement set is $\mathcal{S}=\mathcal{S}_0-\mathcal{S}_a$ where $\mathcal{S}_0$ is the initial full set of measurements and $\mathcal{S}_a$ is the set of removed measurements due to attacks.
Effective detection is constrained to when less than of the measurements of the same controlled state are attacked. Once all temporary inputs $u_i^*(k)$ have been checked, the best available (e.g. measurement with lowest noise profile) is chosen to be used for the true control signal $u_i(k)$ to the system.
