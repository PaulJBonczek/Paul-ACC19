...
\end{figure}
\NB{fix this figure as discussed}
...
\begin{lemma} 
	\label{lemma_1}
	Given $\bm{s}'$ from the measurement set $\bm{y}$, the system is able to maintain tracking before, during, and after a sensor spoof while guaranteeing stability.

\end{lemma}

\begin{proof}
Under the assumptions of (A1-A4) and the convergence properties of \eqref{eq:Modified_Proj_Algorithm}, it follows that:
    \begin{equation}
    \label{lemma_1_eq1}
        \lim_{k\to\infty}e(k)=0
    \end{equation}
With this satisfied, the properties (T1-T3) also hold true. If $e(k)$ is a bounded value for all time instances $k$, then the signal vector $\lVert{\bm{\phi}}(k) \rVert$ is also bounded over time. 

As an attacker hijacks a measurement, \eqref{eq:Psi2_nonideal} ensures a measurement signal never becomes unbounded over all time instances $k$ by removing the measurement from the system. $\lVert{\bm{\phi}}(k) \rVert$ remains bounded, guaranteeing tracking of the desired reference signal.
...
An update of the reference velocity needs to occur when these undesired regions are within the entire confidence estimation region $ C_{\bar{\bm{p}}^*|N_e}+\Delta(k)$ when $N_e=1$ to ensure the vehicle slows to capture more previous data samples with less estimation error. This reference velocity $r_v(k)$ update is calculated by,
    \begin{equation}
        r_v(k) = \min\{\Tilde{r}_v,\bar{r}_v,r_{des}(k)\}
	   % r_v(k)=r_v(k-1) \left[ \frac{ \min \lVert \bm{x}_o - \bar{\bm{p}}^* \rVert - C_1}{C_2 - C_1} \right]
	\end{equation}
where $r_{des}(k)$ is the desired reference and,
	\begin{equation}
	    \Tilde{r} = \frac{d_x}{N_et_s}, \;\;\;\; \bar{r} = \frac{\delta_{\Delta}}{t_s}
	\end{equation}
	\begin{equation}
	    \delta_{\Delta} = \min \lVert \bm{x}_o \rVert - C_{\bar{\bm{p}}^*|N_e}
	\end{equation}
...
\begin{lemma} 
	\label{lemma_2}
Given the present state of the vehicle $\bm{x}(k)$, the vehicle's next state $\bm{x}(k+1)$ will never overlap into an undesired region $\bm{x}_o$.

\end{lemma}

\begin{proof}
With the vehicle presently in a safe state $\bm{x}(k) \neq \bm{x}_o$, there exists a reference signal $r(k)$ such that the vehicle's next state $\bm{x}(k+1)$ will never overlap into an undesired region $\bm{x}_o$. Using \eqref{eq:N_e}, the required number of data samples are computed to guarantee the confidence region $C_{\bar{\bm{p}}(k+1)^*|N_e}$ of the next state $\bm{x}(k+1)$ will never intersect with an undesired state:
\begin{equation}
    C_{\bar{\bm{p}}^*(k+1)|N_e} \cap \bm{x}_o = \emptyset
\end{equation}
Assuming the knowledge of velocity $v(k)$ and distance to the closest obstacle, the reference velocity of the vehicle is calculated such that the vehicle will not travel a distance far enough into an undesired state:
\begin{equation}
    r(k) = \min\{\Tilde{r}_v,\bar{r}_v,r_{des}(k)\}t_s < \min \lVert \bm{x}_o - C_{\bar{\bm{p}}^*(k+1)|N_e} \rVert \nonumber
\end{equation}
Using a closed loop controller to always go towards the trajectory with the assumption the trajectory never crosses through an undesired state.
