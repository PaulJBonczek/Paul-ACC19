...
The purpose of the model reference adaptive control (MRAC) is to maintain a desired control performance when unknown dynamical changes or disturbances occur. Estimation of the system characteristic model's coefficients help give us this desired performance \NB{what do you mean here? What's the main approach? Since it helps}.
Assumptions for adaptive control:\NB{Every sentence must have a subject verb and object}
	\begin{enumerate}[leftmargin=4\parindent]
	\item[$A1)$] all zeros of $B^{'}(z^{-1})z^m$ are within $|z|<1$. \NB{define $z$ before when you are introducing AC model}
	\item[$A2)$] $n$ and $m$ (or their upper bounds) are known. 
	\item[$A3)$] the system delay $d$ is known \NB{is this the same $d$ in the equation (8).} \NB{equations in latex usually have labels so that you can refer to later in the paper}.
	\end{enumerate}
The objective of model reference adaptive control is to calculate an input signal $u(k)$ such that the system tracks $y^{*}(k)$ \NB{is $y$ a scalar or vector?} given the reference signal $r(k)$. 
A following assumption for the reference model is it satisfies\NB{fix grammar here}:
    \begin{enumerate}[leftmargin=4\parindent]
	\item[$A4)$] all zeros of $E(z^{-1})z^p$ are within $|z|<1$. 
	\end{enumerate}

The model reference control is designed (Reference to equation!) by solving the polynomial equation \NB{what does it mean that the reference control is designed by solving?}:
    \begin{equation}
	E(q^{-1})=F(q^{-1})A(q^{-1})+q^{-d}G(q^{-1})
	\end{equation}
By expressing (Ref to equation $A(q^{-1})y(k)=B(q^{-1})u(k)$) \NB{why you are not using the latex environment for referring to equations?..that's why we use latex} as:
	\begin{equation}
...
where,
	...
	\end{align}
we are designing the system to have the same characteristics as the reference model. Parametrization to express this system is:\NB{a reader here is lost. What's happening? Why are you showing this? What's the goal? Explicitly states what we are doing here. Where are the attacks?}
	\begin{equation}
...
where $\bm{\theta}_0$ is unknown and $\bm{\phi}(k)$ is known.
	...
	\end{equation}
with the equation for the input $u(k)$ as \NB{fix: it reads "The adaptive control input $u(k)$ is then calculated from the equation with the equation for the input $u(k)$ as"}:
	\begin{align}
...
\NB{is this first part just adaptive control without attacks? Do we have any contribution here? If not can we just report the final result.}

For detection \NB{of what?}, model reference adaptive control is an effective solution to the problem of dynamically changing or unknown systems. \cite{tao2003adaptive} states that under the assumptions (A1-A4), it ensures \NB{who is it?}:
	...
	\end{enumerate}
This means that the system's output $y(k)$ will asymptotically converge to the tracking reference signal $y^*(k)$ in a finite amount of time. Properties of the modified projection algorithm \NB{what is this algorithm? First time hearing about it in the paper} include,
	...
	\item every iteration improves estimation:
	    ...
	    \end{align}
	    \NB{if you use $k\geq 1$ there is no need to include last inequality}
...
Each system output measurement $\bm{y}_i$ \NB{$y_i \in \bm{y}$ instead} where $i=1,2,\dots,N_y$ and $N_y \in \N$ is the total number of outputs to be measured \NB{fix grammar. sentence is incomplete}. Every $\bm{y}_i$ output measurement has $j$ number of sensors capable of provided data, represented in the form $\bm{y}_{i,j}$ \NB{not clear, need explanation here}. \NB{I'm lost here. Where is the resilient state estimator?}Each of the sensor measurements $\bm{y}_{i,j}$ providing data of each output experience the same convergence behavior to the tracking signal when the sensors are uncompromised. This remains true with a changing reference signal, changing system dynamics, or bounded disturbances. 
With the assurances \NB{what assurances? I think the reference is not correct} of (\ref{assumtions_ensure}), it is true that all uncompromised outputs track its given reference signal via the reference model.
\begin{equation}
\label{multiple_output_tracking}
    \lim_{k\to\infty}(y_{i,j}(k)-y^*_i(k))=0, \text{ }\forall j
\end{equation}
For each $i^{th}$ measured output, a total of $j$ number of MRAC subsystems are built to monitor for dynamical changes and sensor attacks. \NB{Introduce better the following. Why subsystems? What are these subsystems?} Each subsystem receives it's own individual $j^{th}$ sensor signal, but they all share the same reference model, reference signal $r(k)$, and input $u_i(k)$ that is directly used to control the vehicle to a desired output \NB{what is the desired output?}. Each MRAC subsystem calculates an updated input $u_{i,j}$ that are fed into the detector \NB{what is this detector? First time mentioning it}, which analyses each $j^{th}$ input-output combination to check for comprised \NB{?} sensors \NB{how?}. With (\ref{multiple_output_tracking})\NB{wrong reference approach...check our previous papers} ensured, we can also say \NB{rephrase: Since (equation) should hold during any operation, it is also true the follwoing}:
\begin{equation}
...
\label{fig:det_arch}
\end{figure}
\NB{where is this figure mentioned in the text? Every figure needs to be mentioned and described in the text!}
When an attacker maliciously falsifies a sensor signal, the $j^{th}$ output sensor measurement of the $i^{th}$ output state \NB{this is confusing. It took me a while to understand this. Explain better the subsystems} no longer follows the converge rate compared to the uncompromised sensor measurements. As the $j^{th}$ sensor of the $i^{th}$ output no longer follows convergence characteristics, it is removed from the set of uncompromised output sensor measurements $\bm{Y}$ to ensure proper control and estimation for navigation safety \NB{where is the math? So far I saw only previous work and text for our approach}. The new set of output measurements is defined as $\bm{Y}=\bm{Y}_0-\bm{Y}_a$ where $\bm{Y}_0$ is the initial uncompromised output measurement set and $\bm{Y}_a$ is the set of attacked output sensors that have been removed. Effective detection is constrained to when less than $\frac{N_i}{2}$ of sensors are attacked, where $N_i \in \N$ is the total number of available sensors for the $i^{th}$ output.
...
\label{fig:gauss_pdf}
\end{figure}
In this work, the statistical technique of confidence intervals is the foundation of how we solve for estimation \NB{rephrase. In this work we leverage the .... to obtain a confidence about the estimate...and then explain why it is good and what it provides.}. Confidence intervals are used to estimate where the true mean lies in a given stochastic sample of data. 
    \begin{equation}
     \label{Confidence_interval}
		C_I = \bar{x} + z^{*}\frac{\sigma}{\sqrt{N}}
	\end{equation}
%<<<<<<< HEAD
Assuming the knowledge of the confidence percentage $z^{*}$, population standard deviation $ \sigma $, the number of sensor data samples $N$, and the mean of the sensor data samples $ \bar{x} $, a confidence interval of a chosen percentage can be calculated \NB{re-read this section. This sentence should be moved above or changed.}. As the number of samples grow, the radius of the confidence interval shrinks giving a better estimation of the true mean with the same confidence level.
%=======
...
Where its radius is,
	...
	\end{equation}
Similar to the calculation of confidence intervals, it is under the assumption that the true mean, or in our case the vehicle, is not changing positions over the $N$ number of sampled data points used in the confidence interval region calculation \NB{correct, bad grammar}.
...
The unaltered $N$ data samples are described in two sets \NB{why -N?},
\begin{equation}
...
\end{equation}
for the positional coordinate data in the $x$ and $y$ direction \NB{NO, let's use $p=[x,y]$ or similar and group everything in one equation}. Translating the data coordinates into a pseudo-static form will create the updated sets,
\begin{equation}
    \mathcal{\bm{X}}^*=\begin{bmatrix} y_x^*(k-1) ,y_x^*(k-2),\dots,y_x^*(k-N) \end{bmatrix} \nonumber
\end{equation}
\begin{equation}
    \mathcal{\bm{Y}}^*=\begin{bmatrix} y_y^*(k-1) ,y_y(^*k-2),\dots,y_y^*(k-N) \end{bmatrix} \nonumber
\end{equation}
Each element of these updated sets are calculated as \NB{what is $i$? $\forall i \in ...$}:
    \begin{equation}
	...
	\end{equation}
The mean of each set is written as $\hat{\bar{y}}_x^*$ and $\hat{\bar{y}}_y^*$ is used for the calculation of the estimated confidence interval region \NB{read it again, bad grammar}.
...
\NB{what's this algorithm? I cannot find any explanation in the text}
\NB{Same story here as before: proofread the section and improve grammar and make it clear.}
