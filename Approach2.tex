...
Creating a static case with these past position data samples introduces higher uncertainty. Uncertainties regarding the velocity sensor $\sigma_v$ and heading angle sensor $\sigma_h$ need to be accounted for. To guarantee that the true position of the vehicle is within the estimation radial bounds, the maximum error in position due to sensor noise over the furthest sample in time in $\bm{P}^*$ is calculated as:
    \begin{align}
    \begin{split}
	C_{v,\theta_h|N_e}^{*}&=\Big( \big[(\bar{v}^* \cos(c_p\sigma_h) - \bar{v}) \big]^2 \\
	&+ \big[ (\bar{v}^* \sin(c_p\sigma_h) - 0) \big]^2 \Big) ^{1/2} (N_e-1)
	\end{split}
	\end{align}
where,
    \begin{equation}
	...
	\end{equation}
    \begin{equation}
	\bar{v}^{*}=\bar{v}+c_p\sigma_v 
	\end{equation}

with $i_{C^{*}}=1,2,,\dots,N_e-1$ while average measured velocity and average measured heading angles over the past $N_e-1$ samples of range $i_{\varepsilon^{*}}$ are denoted by $\bar{v}$ and $\bar{\theta}_h$, respectively. Accounting for velocity and heading angle measurement error and ensure the vehicle's position is within our estimation region. \NB{simplify this last part. It's confusing.}
