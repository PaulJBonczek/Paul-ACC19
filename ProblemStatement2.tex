...
\begin{enumerate}
	...
	\item remove the compromised sensors and restore system integrity.\NB{need to think about this} %for all future time instances $k+h$. 
\end{enumerate}
	all while adaptive system's control performance.
Solving this problem will allow the autonomous vehicle to safely navigate during system and environmental dynamical changes while preventing an attacker from having an undesirable effect on the system. \NB{rephrase ..system system}
\end{problem}

\NB{Solving for Problem 1 we remove compromised sensors, however the system was designed to use all sensors. Thus we need to replan to consider that we have fewer sensors.}
