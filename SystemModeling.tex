...
When a ground vehicle is in motion, there are many factors that may cause it to behave differently. These factors can include sensor or actuator noises and actuator failure, as well as external influences like wind disturbance, and changing ground surfaces. The uncertainties from actuator noise $\bm{\eta}_a \in \mathcal{N}(0,\sigma_a)$\NB{what is $\mathcal{N}$?} are due to mechanical uncertainties of motors and gears, while disturbances $\bm{d}(k)$ are due to external factors like changes in surface material. The output uncertainty due to measurement noise is described as $\bm{\eta}_s \in \mathcal{N}(0,\sigma_s)$. The measurement uncertainty is assumed to be a truncated Normal distribution with noise bounded at $N_{\sigma}$ standard deviations \NB{why not assume this for everything?}. In this work we consider that an attacker can observe and manipulate some of the sensor measurements by altering their value at any time. The sensor attack vector $\bm{\xi}$ is assumed to be unknown. All noises are assumed to have a zero mean Gaussian distribution\NB{why are you saying this here?}. Putting everything together, the overall discretized system model accounting for noise, disturbance, and attack effects can be described in in the following form:
    \begin{equation}
    \begin{split}
...
The model in \eqref{discrete_SS} can be transformed in a discrete-time transfer function form which is a more usable form for the resilient adaptive control approach later in \ref{sec:Res_adapt_control}. To solve Problem 1 and detect cyber-attacks while adapting the system, we leverage the CMBAC adaptive control approach presented in \cite{4106038}, in which the objective is to compute an input to have the system follow a reference model given changing dynamics (i.e. dynamic $\bm{A_d}$ and  $\bm{B_d}$). Here we summarize the model which will be used later in Section \ref{sec:Res_adapt_control} to adapt the system under uncertainties and to detect attacks. The state space model in \eqref{eq:discrete_SS} transformed in discrete-time transfer function in the $z$-domain becomes:
\NB{states are either bold or not bold, the same goes fro the inputs. Otherwise you need to explain}
	\begin{align}
	\begin{split}
