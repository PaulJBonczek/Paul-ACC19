...
This presents a high-level model of the vehicle, describing only the motion equations. Both forces, $F_l$ and $F_r$, are considered as inputs to the system, are derived from the motors, gearbox, and wheels. All of the resistive parameters, mass, and moment of inertia $I_z$ are able to change to unknown future values as damage, system degradation, and environmental changes occur. The above system model may be rewritten in two separate continuous-time state space systems, the first is for the velocity model:
    \begin{equation}
...
For either system, a general state space model is written in the form,
	...
	\end{equation}
with $\bm{A}(t) \in R^{p\times p}$, $\bm{B}(t) \in R^{p}$, $\bm{x}(t) \in R^{p}$.
...
%where:
	...
	\end{equation}
we can revise (\ref{ARMA_equation}) to the system expressed as:
	\begin{equation}
	A(q^{-1})y(k)=q^{-d}B^{'}(q^{-1})u(k)
	\end{equation}
We want to design an input signal $u(k)$ that ensures our system's output $y(k)$ tracks $y^*(k)$ from the reference model, with a given input reference signal $(k)$. This reference model is expressed as,
	\begin{equation}
	E(q^{-1})y^*(k+d)=q^{-d}gH(q^{-1})r(k)
	\end{equation}
	
to create a stable model reference transfer function in the form:
	\begin{equation}
